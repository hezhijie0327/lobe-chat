...
> = (set, get) => ({
  ...
  searchWithSearXNG: async (id, params, aiSummary = true) => {
    ...
    try {
      data = await searchService.search(params.query, params.searchCategories, params.searchEngines, params.searchTimeRange);

      // 如果没有搜索到结果，那么尝试使用默认的搜索引擎再搜一次
      if (data?.results.length === 0 && (params.searchCategories && params.searchCategories?.length > 0 || params.searchEngines && params.searchEngines?.length > 0 || params.searchTimeRange)) {
        data = await searchService.search(params.query);
        get().updatePluginArguments(id, { ...params, searchCategories: undefined, searchEngines: undefined, searchTimeRange: undefined });
      }
