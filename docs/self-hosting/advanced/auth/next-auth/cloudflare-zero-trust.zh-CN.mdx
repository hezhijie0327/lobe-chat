...
| `CLOUDFLARE_ZERO_TRUST_CLIENT_ID` | 必选 | 刚刚在 Cloudflare Zero Trust 配置的 `id`，示例值是 `lobe-chat` |
| `CLOUDFLARE_ZERO_TRUST_CLIENT_SECRET` | 必选 | 刚刚在 Cloudflare Zero Trust 配置的 `secret` 对应的明文，示例值是 `insecure_secret` |
| `CLOUDFLARE_ZERO_TRUST_ISSUER` | 必选 |您的 Cloudflare Zero Trust 的网址，例如 `https://example.cloudflareaccess.com/cdn-cgi/access/sso/oidc/7db0f` |
